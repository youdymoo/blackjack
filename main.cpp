...
int main() {
    ...
    string playerType;
    bool playerSimple = false;

    bool shuffleMode = false;
    string fileName = "Project-4-Test/test.in";
    ...
    if (!shuffleMode) {
        ...
        inputFile.open(fileName.c_str());
        string shuffleLine;
        getline(inputFile, shuffleLine);
        stringstream shuffleLine_helper(shuffleLine);
        while (shuffleLine_helper){
            shuffleLine_helper >> cut;
            if (shuffleLine_helper && cut >= 0 && cut <= 52) {
                cout << "cut at " << cut << endl;
                deck.shuffle(cut);
            }
        }
...
/*void shuffleDeck(bool shuffleMode, Deck &deck, string fileName) {
    cout << "# Shuffling the deck\n";
    int cut;
    int i;
    if (!shuffleMode) {
